...
function initialize(changePanelVisibilityCallback) {
    registerCustomElement('cordova-panel', {
        ...
        this.shadowRoot.querySelector('.cordova-collapse-icon').addEventListener('click', function (e) {
            var isCurrentlyCollapsed = e.target.classList.contains('cordova-collapsed');

            if (isCurrentlyCollapsed) {
                expandPanel(e.target, content);
            } else {
                collapsePanel(e.target, content);
            }

            if (changePanelVisibilityCallback && typeof changePanelVisibilityCallback === 'function') {
                changePanelVisibilityCallback(panelId, !isCurrentlyCollapsed);
            }
        });
    });
